...
use KobGitUpdater\Updates\UpdateChecker;
...
class Plugin implements PluginInterface {
	...
	public function getOptions(): array {
		if ( $this->options === null ) {
			...
			$this->options = wp_parse_args( is_array( $options ) ? $options : array(), $defaults );
		}
	...
	private function initWordPressHooks(): void {
		...
		register_activation_hook( $this->getPluginFile(), array( $this, 'onActivation' ) );
		register_deactivation_hook( $this->getPluginFile(), array( $this, 'onDeactivation' ) );
		register_uninstall_hook( $this->getPluginFile(), array( self::class, 'onUninstall' ) );
	}
	...
	private function initAdmin(): void {
		...
		$settings_page = $this->container->get( 'admin.settings' );
		$settings_page->init();
	}
	...
	private function initUpdateChecker(): void {
		/** @var UpdateChecker $update_checker */
		$update_checker = $this->container->get( 'update.checker' );
		$update_checker->init();
	}
